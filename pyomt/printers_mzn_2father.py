...
##Try to implement the rewriting only when the two fathers are differents
class SmtDagPrinter(DagWalker):
    
    def __init__(self,max_int_bit_size,stream,template="tmp_%d",boolean_invalidate=True):
...
class MZNPrinter(object):
    ...
    def serialize(self,formula,daggify=True,output_file=None):
        ...
        str_let_list=[]
        if dict_f:
            #p = DagWalkerFathers(self.max_int_bit_size,buf,{},boolean_invalidate=False)
            k=dict_f.keys()
            ...
            for el in k:
                ...
                #print "calling ",el," con mem",dict_f
                p.printer(el)
        ...
        buf.close()
        #for each element in dict which have more than 2 apply subs
        #poi chiamare il dagWalker su fathe
        '''
        buf = cStringIO()
        if daggify:
            p = SmtDagPrinterNoPrint(self.max_int_bit_size,buf)
        else:
            p = HRPrinter(buf)

        dict_2f=None
        dict_2f=p.printer(formula)
        if dict_2f is not None:
            #sub
            buf = cStringIO()
            ...
            flag=0
            dict_sub={}
            fm=get_env()._formula_manager
            str_a=""
            for el in dict_2f:
                if "tmp" in str(dict_2f[el]):
                    if flag==0:
                        str_a += "let { \n"
                        flag=1
                    #buf = cStringIO()
                    ...
                    p.printer(el)
                    str_a += " var %s : %s = ( %s ) \n "%(str(el.get_type()).lower().replace("real","float"),dict_2f[el],buf.getvalue())

                    ns=fm._create_symbol(dict_2f[el],el.get_type())
                    dict_sub[el]=ns
            str_a += "} in \n"
            buf = cStringIO()
            buf.write(str_a)
            #p = SmtDagPrinter(self.max_int_bit_size,buf,boolean_invalidate=False)
            p = HRPrinter(buf)
            formula=sh.substitute(formula,dict_sub)    
            p.printer(formula)        
        res=buf.getvalue()
        if output_file is None:
            return res
