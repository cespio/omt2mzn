...
from pyomt.exceptions import (SolverReturnedUnknownResultError,
                              ...
                              UndefinedSymbolError, PyomtValueError)
from pyomt.decorators import clear_pending_pop, catch_conversion_error
from pyomt.logics import LRA, LIA, QF_UFLIA, QF_UFLRA, PYSMT_LOGICS
from pyomt.oracles import get_logic
from pyomt.constants import Fraction, Numeral, is_pyomt_integer, to_python_integer
...
class Z3Model(Model):
    ...
    def __iter__(self):
        ...
        for d in self.z3_model.decls():
            if d.arity() == 0:
                try:
                    pyomt_d = self.converter.back(d())
                    yield pyomt_d, self.get_value(pyomt_d)
...
class Z3Converter(Converter, DagWalker):
    ...
    def back_via_smtlib(self, expr):
        ...
        from pyomt.smtlib.parser import SmtLibZ3Parser
...
class Z3QuantifierEliminator(QuantifierEliminator):
    ...
    def eliminate_quantifiers(self, formula):
        ...
        if not logic <= LRA and not logic <= LIA:
            raise PyomtValueError("Z3 quantifier elimination only "\
        ...
        pyomt_res = None
        try:
            pyomt_res = self.converter.back(res)
...
class Z3Interpolator(Interpolator):
    ...
    def _check_logic(self, formulas):
        for f in formulas:
            ...
            if not ok:
                raise PyomtValueError("Logic not supported by Z3 interpolation."
                                      "(detected logic is: %s)" % str(logic))
    ...
    def binary_interpolant(self, a, b):
        ...
        try:
            ...
            pyomt_res = self.converter.back(itp)
        except z3.ModelRef:
            pyomt_res = None
    ...
    def sequence_interpolant(self, formulas):
        ...
        try:
            ...
            pyomt_res = [self.converter.back(f) for f in itp]
        except z3.ModelRef:
            pyomt_res = None
