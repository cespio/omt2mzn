...
class Parser():
    ...
    def __init__(self):
        self.serializer=HRSerializer()

    def pre_proc(self,input_file):
        '''
    ...
    def modify_type_assert_soft(self,asserts_soft_list,var_dict):
        '''
  ...
  [a[k] | k in new_j..new_i];\n""")
    ...
    predicate sumBV(array[int] of var bool: x,
        ...
        for el in asserts_list:
            if type(el) is list:
                el=el[0]       
            self.serializer.serialize_assert(el,file_out)
    ...
    def write_stack(self,stack,out_file):
        ...
        var_dict=self.modify_type_assert_soft(asserts_soft_list,var_dict) #TODO:review
    ...
    def parse_stack(self,commands,out_file):
        ...
        for cmd in commands:
            if cmd.name=='push':
                ...
                self.write_stack(current_stack,out_file.replace(".mzn","_"+str(file_index)+".mzn"))
                file_index+=1
            else:
                current_stack[-1].append(cmd)
            
                
    def startParsing(self,input_file,out_file):
        parser = SmtLib20Parser() 
        new_input_f = self.pre_proc(input_file)
        ...
        self.parse_stack(commands,out_file)  #calling the main function
...
if __name__ == "__main__":
    if len(sys.argv)!=3:
        ...
        parser=Parser()
        parser.startParsing(sys.argv[1],sys.argv[2])